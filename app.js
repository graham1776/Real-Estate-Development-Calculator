...
function updateMetrics() {
  ...
  drawRectangles(FAR, buildingSqft, landAc);
	updateBarChart(yieldOnCost, returnOnCost);
}
...
function updateBarChart(yieldOnCost, returnOnCost) {
  ...
  barChart = new Chart(ctx, {
    type: 'bar',
    data: {
      labels: ['Yield on Cost', 'Return on Cost'],
      datasets: [{
        label: 'Percentage',
        data: [yieldOnCost, returnOnCost * 100],
        backgroundColor: [
          'rgba(75, 192, 192, 0.6)',
          'rgba(255, 99, 132, 0.6)'
        ],
